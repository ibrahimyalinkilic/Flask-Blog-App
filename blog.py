    ...
    if request.method == "POST":
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(password_entered, real_password):
                flash("Giriş başarılı. Hoşgeldiniz: {}".format(
                    data["name"]), "success")
...
def addarticle():
    ...
    if request.method == "POST" and form.validate():
        ...
        mysql.connection.commit()
        cursor.close()
        flash("Başarıyla makalenizi eklediniz.","success")
...
def search():
    if request.method=="GET":
        ...
        if result == 0 :
            flash("Aranan keliemeye uygun makale bulunmamaktadır.","warning")
...
def profile():
    cursor = mysql.connection.cursor()
    sorgu = "Select name from users where username=%s"
